...
- `PFD_PostgreSQL_Database_Build_and_Change-Control_Plan.md`

---

## 1. Purpose

This specification defines the concrete repository structure and first PostgreSQL database build package for PFD.
...
- PFD database ownership structure
...
2. PFD uses one continuing business database per environment.
...
Contains cluster- and database-level setup that cannot be executed as an ordinary change inside the not-yet-created PFD database.
...
Bootstrap creates the minimum PostgreSQL objects required before permanent PFD changes can run.
...
10. Obtains the PFD database-build advisory lock.
...
Records the exact permanent SQL history applied to a PFD database.
...
Fails when the PFD schemas contain:
...
- `PUBLIC` cannot create in PFD schemas.
...
The next deliverable should be the **PFD PostgreSQL Core Build SQL Package**.
...
This document completes the detailed repository and Core Build specification for PFD as of September 4, 2026.

...
This document translates PFD's approved logical information model and transaction lifecycles into a relational persistence architecture based on PostgreSQL.
...
PostgreSQL is the selected persistent data platform for PFD.
...
1. Represent the approved PFD business model faithfully.
...
PFD uses one logical application database per environment:
...
PFD shall organize database objects into functional schemas.
...
- Currency is explicit even while PFD operates solely in USD.
...
PFD's simulation operates one persistent business database in the same manner as a production application. A session may advance a day, a week, or another controlled interval, but the next session continues from the preceding period's ending inventory, cash, receivables, payables, orders, commitments, payroll, assets, debt, and general ledger.
...
PFD stores Inventory Lot through receiving, location, and placement history but does not store exact Lot on customer shipment lines. The relational model shall not imply precision PFD does not operationally capture. Recall exposure uses product, time-window, movement, and remaining-balance analysis and is labeled estimated.
...
PFD's opening scale does not require every transaction table to be partitioned. Normal indexed tables are the default.
...
Business authorization remains application-aware because PostgreSQL roles alone do not express all PFD approvals, thresholds, and workflow states. Database privileges and business-role authorization work together.

### 34.4 Row-level security

Row-level security is not required for PFD's single-company opening model. It may be introduced only if a future multi-tenant or strong row-segregation requirement exists. Test copies are isolated at the database/environment level rather than by row-level Simulation Session keys.
...
The next deliverable should be the **PFD Relational Schema and Table Definition Specification**.
...
This document establishes the persistent relational architecture and PostgreSQL database standards for PFD as of September 4, 2026.

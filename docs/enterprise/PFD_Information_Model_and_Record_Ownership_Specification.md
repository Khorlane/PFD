...
This document defines PFD's logical business records and establishes who owns, creates, approves, changes, and consumes them.
...
PFD must be able to answer both:
...
|---|---|---|---|---|
| Company | Company Number | General Management | Administration | Legal and operating identity of PFD |
...
- PFD owns one opening Facility at \<business address>.
...
PFD does not store an exact Inventory Lot reference on each customer shipment line. Customer recall exposure is estimated from:
...
| Driver Return | Driver Return Number | Driver/Warehouse | Product returned to PFD custody after route |
...
| Return Receipt | Return Receipt Number | Warehouse | Physical return to PFD custody |
...
- Temperature-controlled goods that left PFD control are presumed unsuitable for resale unless integrity is established.
...
| Class | Information | Minimum PFD retention policy |
|---|---|---|
...
At restart, PFD must identify records that were:
...
The following catalog summarizes the authoritative PFD logical records.
...
The next deliverable should be the **PFD Business Process and Transaction Lifecycle Specification**.
...
This document completes the logical information-model and record-ownership layer for the approved PFD business model as of September 3, 2026.

...
This plan defines how the PostgreSQL database will be created, validated, changed, promoted, recovered, and documented.

This is a new system. There is no legacy database conversion implied by this plan. The first set of SQL files builds the database from an empty PostgreSQL database. Later files make controlled changes without rebuilding or manually editing an authoritative database.
...
The complete ordered execution of SQL files against an empty PostgreSQL database to create a usable database.
...
The continuing business database whose records carry forward from one simulated day or week to the next. It is production-shaped even while the business is simulated.
...
| Authoritative Simulation | Continuing business operation | Authoritative simulated business records | Never routinely reset |
...
- `opening-data` contains the approved initial master and financial data, not ordinary ongoing transactions.
...
Every proposed release builds a new database from empty using all ordered files. This proves that a new environment can be recreated without undocumented manual actions.
...
1. A new database can be built from empty without manual structural edits.
...
The next deliverable should be the **Database Repository and Core Build Specification**.
...
This document completes the implementation plan for creating and controlling the PostgreSQL database as of September 4, 2026.

...
**Depends on:** design through the Finance and Accounting PostgreSQL Build Specification

## 1. Purpose

Define how the business plans, hires, assigns, schedules, qualifies, pays, develops, and separates its workforce. The domain must connect employee availability and competence to operating capacity while producing complete, approved payroll results for Finance without duplicating identity, operational, banking, or accounting facts.

## 2. Required Outcomes

- The business knows who may work, in what role, at which location, on which shift, and under which qualifications.
...
The business opens with approximately **45–50 employees, including any owners who also hold Employee roles**, supporting approximately 80 Customer locations, 3,000 Products, a 50,000-square-foot facility, and six owned delivery trucks.
...
The business distinguishes employment status from payroll and schedule status. Employment states include candidate, offered, prehire, active, leave, suspended, separated, and deceased. Payroll eligibility and operational eligibility are separately derived.
...
The business pays all compensable work required by applicable law and policy. Failure to obtain advance approval is handled as a performance matter, not by deleting or refusing valid worked time.
...
The business uses one company-wide **biweekly payroll calendar** for hourly employees, salaried employees, and owners on payroll. Each Payroll Period spans two defined workweeks and has a published time cutoff, manager approval cutoff, Payroll processing date, payment date, and correction cutoff.
...
The business reconciles enrollment, Payroll deductions, employer contributions, carrier invoices, and Finance remittance. The opening plan catalog and contribution amounts are configuration, not unresolved architecture.
...
The business separates routine Employee information from restricted tax identifiers, banking instructions, medical/accommodation facts, background results, garnishments, benefit dependents, and investigation material.
...
During system interruption, the business may use controlled numbered schedule, attendance, time, hiring, and payroll documents. Emergency payroll requires explicit authority, known funding, preserved source evidence, and later independent reconciliation.
...
- One biweekly payroll calendar for all Employees and owners on Payroll.
...
Next: **Workforce and Payroll PostgreSQL Build Specification**. It will define normalized PostgreSQL structures, natural keys, controlled functions, constraints, privileges, indexes, views, verification, and tests without executable SQL.

...
**Depends on:** PostgreSQL Core Build through change `0010`; Party and Customer Domain Specification

---

## 1. Purpose

This specification defines the PostgreSQL database objects required to implement Party and Customer domain. It is the contract for the subsequent executable SQL package.
...
The implementation targets PostgreSQL 15 or later on a vendor-supported release. It extends the database created by the PostgreSQL Core Build.
...
All changes execute under `SET LOCAL ROLE pfd_database_owner` inside a transaction controlled by the existing build runner.
...
Views expose current effective records using database time and the timezone where required. They do not hide historical base tables from authorized support or audit roles.
...
Provider-specific identifiers must be stored as alternate identifiers or integration configuration. They never replace Party, Customer, Address, or Location keys.

## 42. Approved Next Step

After review of this specification, the next deliverable is the **Party and Customer PostgreSQL Build SQL Package**, containing the cumulative manifest, permanent changes `0011` through `0022`, reference CSVs, verification, behavioral tests, and operating documentation.

...
**Depends on:** Cumulative PFD design through `0032`
...
`purchase_order` is the stable header keyed by `purchase_order_number`; it stores Supplier, ship-from, PFD destination, buyer, current status/revision, creation date, and audit data.
...
Next: **PFD Inventory Domain Specification**. Executable Supplier/Purchasing SQL remains deferred until PFD leaves Design Land.

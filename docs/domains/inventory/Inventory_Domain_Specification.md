...
**Depends on:** Core, Product, Supplier/Purchasing, and future Warehouse transaction boundaries

## 1. Purpose

Define how the business accounts for physical stock from receipt through allocation, movement, shipment, return, adjustment, and disposition. This is business and logical-data design, not PostgreSQL implementation.
...
Every lot-controlled receipt receives an Inventory Lot Number. It links:
...
Supplier lot is not the key because formats may repeat across Suppliers or Products.
...
`Product + Lot + Pallet + Warehouse Location + Stock Status + Ownership`
...
For FEFO, the earliest qualifying expiration/use-by/best-by date is selected. Ties use receipt time and then Lot Number. FIFO uses accepted receipt time and then Lot Number.
...
A hold records scope, type, reason, effective time, initiating Principal, approval, and release conditions. Scope may be Product, Supplier/manufacturer lot, lot, pallet, location, or specific quantity.
...
Next design deliverable: **Inventory PostgreSQL Build Specification**. It will define normalized tables, business keys, constraints, indexes, functions, privileges, verification, and tests without producing executable SQL.

...
**Depends on:** Cumulative PFD design through `0044`; PFD Inventory Domain Specification
...
- All inventory uses PFD Inventory Lot identity, even when Supplier lot capture is unnecessary.
...
Every accepted stock quantity receives a PFD Inventory Lot. For Products with lot method `NONE`, the PFD lot still identifies receipt/cost provenance; Supplier/manufacturer lot fields remain null.
...
Scope tables use meaningful composites for Product, Supplier/manufacturer lot, PFD lot, pallet, location, or quantity. Hold placement atomically moves affected available quantity to the correct hold status and identifies existing reservations/allocations for review.
...
Database views calculate remaining days from the governing lot date and PFD business date. Allocation functions apply Product policy and stricter Customer/Product rules.
...
Next: **PFD Warehouse Operations Domain Specification**. Executable Inventory SQL remains deferred until PFD leaves Design Land.

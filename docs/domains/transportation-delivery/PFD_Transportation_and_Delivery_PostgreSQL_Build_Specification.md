...
**Depends on:** Cumulative PFD design through `0082`; PFD Transportation and Delivery Domain Specification
...
`transportation.fuel_event` PK `truck_number + event_time + fuel_ticket_reference`; stores odometer, fuel quantity/unit/type, vendor/location, amount/currency, purchaser/card reference, source, and exception. A manual event uses a controlled PFD fuel-ticket reference rather than a null or fake key.
...
Required-proof failure creates a Delivery Exception. Evidence storage follows PFD File Standards; large signatures/images are not embedded indiscriminately in operational rows.
...
Loose refused goods must receive a physical PFD Return Handling Unit before the first custody event; the identifier represents the actual controlled container, not a placeholder.
...
Next: **PFD Finance and Accounting Domain Specification**. Executable Transportation and Delivery SQL remains deferred until PFD leaves Design Land.

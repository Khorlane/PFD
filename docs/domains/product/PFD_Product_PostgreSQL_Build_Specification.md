...
**Depends on:** Cumulative PFD database design through change `0022`; PFD Product Domain Specification

## 1. Purpose

Define the PostgreSQL structures and controls required to implement PFD Product management. This is a design contract for a later SQL package; it does not modify a database.
...
| `product.lot_control_method` | `NONE`, `SUPPLIER_LOT`, `PFD_LOT`, `SUPPLIER_AND_PFD_LOT` |
| `product.rotation_method` | `FEFO`, `FIFO`, `MANUAL_APPROVAL` |
| `product.identifier_type` | `GTIN_14`, `UPC_A`, `EAN`, `MANUFACTURER_ITEM`, `LEGACY_PFD` |
...
| `product.measurement_source` | `MANUFACTURER`, `SUPPLIER`, `PFD_VERIFIED` |
...
After this specification, the next design document should be the **PFD Supplier and Purchasing Domain Specification**. Executable Product SQL remains deferred until PFD leaves Design Land.
